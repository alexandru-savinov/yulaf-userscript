...
const fixtureUrl = 'file://' + path.resolve(__dirname, 'youtube-shorts.html');

test('shorts feed: reel-item-renderer items are processed', async ({ page }) => {
  const errors = [];
...
test('shorts feed: english visible, non-english hidden', async ({ page }) => {
  await injectUserscript(page);
  await page.goto(fixtureUrl);
