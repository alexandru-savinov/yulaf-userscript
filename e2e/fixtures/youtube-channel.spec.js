...
const fixtureUrl = 'file://' + path.resolve(__dirname, 'youtube-channel.html');

test('channel page: grid-video-renderer items are processed', async ({ page }) => {
  const errors = [];
...
test('channel page: english visible, non-english hidden', async ({ page }) => {
  await injectUserscript(page);
  await page.goto(fixtureUrl);
