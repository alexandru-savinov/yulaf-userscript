...
const fixtureUrl = 'file://' + path.resolve(__dirname, 'youtube-watch.html');

test('watch sidebar: compact-video-renderer items are processed', async ({ page }) => {
  const errors = [];
...
test('watch sidebar: english visible, non-english hidden', async ({ page }) => {
  await injectUserscript(page);
  await page.goto(fixtureUrl);
