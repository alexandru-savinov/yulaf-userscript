...
const fixtureUrl = 'file://' + path.resolve(__dirname, 'youtube-subscriptions.html');

test('subscriptions: rich grid items are processed', async ({ page }) => {
  const errors = [];
...
test('subscriptions: english visible, ru/ja/tr hidden', async ({ page }) => {
  await injectUserscript(page);
  await page.goto(fixtureUrl);
